...
do {									\
	chip->irq_name##_irq.irq = of_irq_get_byname(node,		\
					#irq_name);			\
				...
				struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(node, child) {
		...
		if (strcmp("qcom,batt-pres-status",
					child->name) == 0) {
			chip->batt_pres_addr = (u16)base;
			continue;
		}

		if (strcmp("qcom,qpnp-chg-pres",
					child->name) == 0) {
			chip->chg_pres_addr = (u16)base;
			continue;
		}
...
static const struct power_supply_desc bms_psy_desc = {
	...
	.properties		= bms_power_props,
	.num_properties		= ARRAY_SIZE(bms_power_props),
	.get_property		= qpnp_vm_bms_power_get_property,
	.set_property		= qpnp_vm_bms_power_set_property,
	.external_power_changed	= qpnp_vm_bms_ext_power_changed,
	.property_is_writeable	= qpnp_vm_bms_property_is_writeable,
};
...
static int qpnp_vm_bms_probe(struct platform_device *pdev)
{
	...
	rc = parse_spmi_dt_properties(chip, pdev);
	if (rc) {
	...
	/* setup & register the battery power supply */
	{
		struct power_supply_config bms_cfg = {
			...
			.supplied_to = qpnp_vm_bms_supplicants,
			.num_supplicants = ARRAY_SIZE(qpnp_vm_bms_supplicants),
		};

		chip->bms_psy = power_supply_register(chip->dev,
				&bms_psy_desc, &bms_cfg);
		if (IS_ERR(chip->bms_psy)) {
			rc = PTR_ERR(chip->bms_psy);
...
{
	return platform_driver_register(&qpnp_vm_bms_driver);
}
module_init(qpnp_vm_bms_init);

static void __exit qpnp_vm_bms_exit(void)
{
	platform_driver_unregister(&qpnp_vm_bms_driver);
}
