 ...
 * @regmap:			regmap pointer for SPMI register access
...
static int __qpnp_lbc_read(struct regmap *regmap, u16 base,
			u8 *val, int count)
{
	int rc = 0;

	rc = regmap_bulk_read(regmap, base, val, count);
	if (rc)
...
static int __qpnp_lbc_write(struct regmap *regmap, u16 base,
			u8 *val, int count)
{
	int rc;

	rc = regmap_bulk_write(regmap, base, val, count);
	if (rc)
...
static int __qpnp_lbc_secure_write(struct regmap *regmap, u16 base,
				u16 offset, u8 *val, int count)
{
	...
	reg_val = 0xA5;
	rc = __qpnp_lbc_write(regmap, base + SEC_ACCESS, &reg_val, 1);
	if (rc) {
		pr_err("SPMI read failed base=0x%02x rc=%d\n",
				base + SEC_ACCESS, rc);
		return rc;
	...
	rc = __qpnp_lbc_write(regmap, base + offset, val, 1);
	if (rc)
		pr_err("SPMI write failed base=0x%02x rc=%d\n",
				base + SEC_ACCESS, rc);
			...
			u8 *val, int count)
{
	...
	if (base == 0) {
		pr_err("base cannot be zero base=0x%02x rc=%d\n", base, rc);
		return -EINVAL;
	}

	spin_lock_irqsave(&chip->hw_access_lock, flags);
	rc = __qpnp_lbc_read(chip->regmap, base, val, count);
			...
			u8 *val, int count)
{
	...
	spin_lock_irqsave(&chip->hw_access_lock, flags);
	rc = __qpnp_lbc_write(chip->regmap, base, val, count);
	spin_unlock_irqrestore(&chip->hw_access_lock, flags);
				...
				u8 mask, u8 val)
{
	...
	spin_lock_irqsave(&chip->hw_access_lock, flags);
	rc = __qpnp_lbc_read(chip->regmap, base, &reg_val, 1);
	if (rc) {
	...
	rc = __qpnp_lbc_write(chip->regmap, base, &reg_val, 1);
	if (rc)
...
static int __qpnp_lbc_secure_masked_write(struct regmap *regmap, u16 base,
				u16 offset, u8 mask, u8 val)
{
	int rc;
	u8 reg_val, reg_val1;

	rc = __qpnp_lbc_read(regmap, base + offset, &reg_val, 1);
	if (rc) {
		pr_err("spmi read failed: addr=%03X, rc=%d\n", base, rc);
		return rc;
	}
	...
	reg_val1 = 0xA5;
	rc = __qpnp_lbc_write(regmap, base + SEC_ACCESS, &reg_val1, 1);
	if (rc) {
		pr_err("SPMI read failed base=0x%02x rc=%d\n",
				base + SEC_ACCESS, rc);
		return rc;
	...
	rc = __qpnp_lbc_write(regmap, base + offset, &reg_val, 1);
	if (rc) {
		pr_err("SPMI write failed base=0x%02x rc=%d\n",
				base + offset, rc);
		return rc;
		...
		bool enable)
{
	...
	if (enable) {
		...
		rc = __qpnp_lbc_secure_masked_write(chip->regmap,
...
static int qpnp_lbc_vddmax_set(struct qpnp_lbc_chip *chip, int voltage)
{
	...
	reg_val = (voltage - QPNP_LBC_VBAT_MIN_MV) / QPNP_LBC_VBAT_STEP_MV;
	pr_debug("voltage=%d setting %02x\n", voltage, reg_val);
	rc = __qpnp_lbc_write(chip->regmap, chip->chgr_base + CHG_VDD_MAX_REG,
				&reg_val, 1);
	if (rc) {
...
static int qpnp_lbc_vbatdet_override(struct qpnp_lbc_chip *chip, int ovr_val)
{
	...
	rc = __qpnp_lbc_read(chip->regmap, chip->chgr_base + CHG_COMP_OVR1,
				&reg_val, 1);
	if (rc) {
		pr_err("spmi read failed: addr=%03X, rc=%d\n",
						chip->chgr_base, rc);
	...
	rc = __qpnp_lbc_secure_write(chip->regmap, chip->chgr_base,
					CHG_COMP_OVR1, &reg_val, 1);
	if (rc)
		pr_err("spmi write failed: addr=%03X, rc=%d\n",
						chip->chgr_base, rc);
...
#define SPMI_GET_IRQ_RESOURCE(chip, rc, node, idx, name)			\
do {									\
	if (rc)								\
		break;							\
									\
	rc = of_irq_get_byname(node, #name);				\
	if (rc < 0) {							\
					...
					struct device_node *child)
{
	...
	case LBC_CHGR_SUBTYPE:
		SPMI_GET_IRQ_RESOURCE(chip, rc, child,
...
static int qpnp_lbc_parallel_charger_init(struct qpnp_lbc_chip *chip)
{
	...
	reg_val = 0x2A;
	rc = __qpnp_lbc_secure_write(chip->regmap, chip->bat_if_base,
...
static int qpnp_lbc_parse_resources(struct qpnp_lbc_chip *chip)
{
	...
	for_each_available_child_of_node(node, child) {
		...
		if (rc) {
			pr_err("node %s reg property absent!\n",
					child->full_name);
			return -ENXIO;
		}

		rc = qpnp_lbc_read(chip, (u16)base + PERP_SUBTYPE_REG,
								&subtype, 1);
		if (rc) {
...
static const struct power_supply_desc parallel_psy_desc = {
	...
	.properties	= qpnp_lbc_parallel_properties,
	.num_properties	= ARRAY_SIZE(qpnp_lbc_parallel_properties),
	.get_property	= qpnp_lbc_parallel_get_property,
	.set_property	= qpnp_lbc_parallel_set_property,
...
static const struct power_supply_desc batt_psy_desc = {
	...
	.get_property	= qpnp_batt_power_get_property,
	.set_property	= qpnp_batt_power_set_property,
	.property_is_writeable = qpnp_batt_property_is_writeable,
	.external_power_changed = qpnp_batt_external_power_changed,
};
...
static int qpnp_lbc_parallel_probe(struct platform_device *pdev)
{
...

	{
		...
		chip->parallel_psy = power_supply_register(chip->dev,
				&parallel_psy_desc, &parallel_cfg);
		if (IS_ERR(chip->parallel_psy)) {
			rc = PTR_ERR(chip->parallel_psy);
