...
static void bam_recv_notify(void *dev, struct sk_buff *skb)
{
	...
	if (skb) {
...
#endif
			((struct net_device *)dev)->stats.rx_packets++;
			((struct net_device *)dev)->stats.rx_bytes += skb->len;
		}
		DBG1("[%s] Rx packet #%lu len=%d\n",
			((struct net_device *)dev)->name,
			((struct net_device *)dev)->stats.rx_packets, skb->len);
		...
		if (pkt_threshold == 1) {
			...
			if (((struct net_device *)dev)->stats.rx_packets % pkt_threshold == 0)
				netif_rx_ni(skb);
			else
				netif_rx(skb);
		}
...
static void bam_write_done(void *dev, struct sk_buff *skb)
{
	...
	if (RMNET_IS_MODE_IP(opmode) ||
				count_this_packet(skb->data, skb->len)) {
		((struct net_device *)dev)->stats.tx_packets++;
		((struct net_device *)dev)->stats.tx_bytes += skb->len;
#ifdef CONFIG_MSM_RMNET_DEBUG
		p->wakeups_xmit += rmnet_cause_wakeup(p);
#endif
	}
	DBG1("[%s] Tx packet #%lu len=%d mark=0x%x\n",
	    ((struct net_device *)(dev))->name, ((struct net_device *)dev)->stats.tx_packets,
	    skb->len, skb->mark);
...
static int rmnet_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case RMNET_IOCTL_FLOW_DISABLE:
		/* Flow control requires CONFIG_NET_SCHED (tc_qdisc_flow_control) */
		rc = -EOPNOTSUPP;
		break;
