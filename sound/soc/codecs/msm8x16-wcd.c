...
static int msm8x16_wcd_spmi_write_device(u16 reg, u8 *value, u32 bytes)
{
	...
	ret = spmi_ext_register_writel(wcd->spmi,
...
int msm8x16_wcd_spmi_read_device(u16 reg, u32 bytes, u8 *dest)
{
	...
	ret = spmi_ext_register_readl(wcd->spmi,
...
static int __maybe_unused msm8x16_wcd_volatile(struct snd_soc_codec *codec, unsigned int reg)
{
...
static int __maybe_unused msm8x16_wcd_readable(struct snd_soc_codec *ssc, unsigned int reg)
{
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_err(snd_soc_dapm_to_codec(w->dapm)->dev, "%s %d %s\n", __func__, event, w->name);
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(snd_soc_dapm_to_codec(w->dapm)->dev, "%s event %d w->name %s\n", __func__,
			event, w->name);
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		msm8x16_wcd_codec_enable_clock_block(codec, 1);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		dev_dbg(snd_soc_dapm_to_codec(w->dapm)->dev,
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(snd_soc_dapm_to_codec(w->dapm)->dev, "%s: %d %s\n", __func__, event, w->name);
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec,
			MSM8X16_WCD_A_CDC_RX3_B6_CTL, 0x01, 0x00);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		dev_err(snd_soc_dapm_to_codec(w->dapm)->dev,
